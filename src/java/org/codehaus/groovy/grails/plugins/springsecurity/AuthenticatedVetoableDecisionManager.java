...
public class AuthenticatedVetoableDecisionManager extends AbstractAccessDecisionManager {
	 ...
	 */
	private boolean checkAuthenticatedVoters(Authentication authentication, Object object, ConfigAttributeDefinition config) {
		boolean grant = false;
	 ...
	 */
	private boolean checkOtherVoters(Authentication authentication, Object object, ConfigAttributeDefinition config) {
		int denyCount = 0;
		for (AccessDecisionVoter voter : getVoters()) {
			if (voter instanceof AuthenticatedVoter) {
				continue;
	...
	private List<AccessDecisionVoter> getVoters() {
		return getDecisionVoters();
	}
